 ...
 * │   → 严格分层决策树 L0~L15：每层触发即返回（层内用            │
 * │     pickBestBySearch 对候选点深搜选优）；全不触发 → L15 深搜  │
 * └──────────────────────────────────────────────────────────────┘
 *
 * 代码按决策树逻辑分节组织：
 ...
 *   八、分层检测         L2~L15 各层检测函数（按决策树执行顺序排列）
 *   九、启发式搜索       genMoves（着法排序）+ negamax（α-β 深搜）+ pickBestBySearch
 *   十、总控             getMove（L0~L15 分层漏斗，决策树主入口）
 *   十一、API 导出
 *
 ...
 *   L6   己方VCF连续冲四杀      vcfWin（2026-08-14 提升到防一步必胜前：连续冲四必胜，压制对手一步威胁）
 *   L7   防一步必胜威胁        blockCombined（is43/is44/活四 + 冲四+潜在四三四四≥P_D43）
 *   L8   防三三攻防一体        findDefendWithRush4（2026-08-18 与 L9 交换：三三明显、VCF 隐蔽）
 *   L9   防对方VCF杀           findOppVcfDefense（连续冲四必胜，原 L8）
 *   L9-b 敌方VCF路上冲四+潜在真三三  findVcfTriple33Block（2026-08-18 新增，L9 防VCF 子类）
 *   L10  己方三三              findOwn33
 *   L11  己方做杀层            findOwnKillLayer（冲四+半径4三三；活三+半径4四三四四三三）
 *   L12  防对方冲四+潜在三三    blockRush4Potential33（非一步必胜，放己方三三/做杀后）
 *   L13  防潜在三三            blockPotential33（含"活三+做杀"向后看，半径4）
 *   L14  开局决策层            openingDecision（前7手定式：合并原 L1/L9/L12b）
 *   L15  启发式深搜            genMoves + negamax 迭代加深 1→MAX（分桶截断仅白棋：第一层cap11、内部cap10；黑棋纯分数）
 *
 * 开局决策层（L14，前7手）：
 *   第1手(黑) 天元
 ...
 *   执黑 = 进攻导向（保留进攻价值、堵白降权×0.7 仅"白威胁不占优"且仅 L15 启发层；
 *           攻守转换时黑正常防守围堵，不能丢防守）
 *
 * API：
...
(function (global) {
  ...
  // 斜线激励（仅极早生效，见 getMove L14 开局决策层与 genMoves stones<4 分支）：
  ...
  // 分数从 color 视角（越高越有利）。
  // ⚠ applyLst=true 仅 L15 启发式层（顶层 genMoves 传入）；强制层深搜传 false，
  ...
  function evaluateBoard(board, N, color, applyLst) {
    ...
    //   黑 → 堵白评估降权×0.7 仅"白威胁不占优"（黑主动进攻阶段）且 L15 启发层（applyLst）生效；
    ...
    if (color === BLACK && applyLst) {
      ...
      // ⚠ 2026-08-18 增强防御：敌人未近身搏杀 → 跳过前 9 手强制进攻（按白威胁正常攻守转换）。
      if (!_enhancedDefense && countStones(board, N) <= 9) defMul = 0.7;
  ...
  //   ② 冲四(RUSH4) + potentialThreat >= P_D43 → 冲四 + 下一步四三/四四（两步必胜），同 L7 级
  // ⚠ 2026-08-14 用户：**冲四 + 潜在三三（P_D33）不属于一步必胜**——我方可先下己方三三再应对，
  //   已从 L7 分离到"防对方冲四+潜在三三"层（blockRush4Potential33，放己方三三/做杀之后）。
  ...
  function blockCombined(board, N, me, opp) {
    ...
    for (i = 0; i < focus.length; i++) {
      ...
      // 往后看：敌人下 P 形成冲四(RUSH4)，且落子后为下一步四三/四四铺路
      // （potentialThreat >= P_D43）→ 冲四+两步必胜，与一步必胜同级
      else if (t.tier === T_RUSH4 && potentialThreat(board, N, focus[i], opp, 4, true) >= V.P_D43) {
        T.push(focus[i]);
    ...
    // ⚠ 2026-08-18 用户（方向修正，替代初版"选安全端点"——初版误排除带杀招端点致棋力下降）：
    //   活三端点候选**优先占"对手落该端点带杀招"的点**（阻断对手做杀）——
    //   (1) 对手落 a 成冲四 + 附近潜在四四/四三/真三三（1步杀招）优先堵 a；
    //   (2) 对手落 a 冲四 → 附近 3 格内冲三 → 无论堵新冲三哪边都有做杀（2步杀招）其次堵 a；
    //   (3) 多个端点都带杀招（2个点都有）或都不带 → 交 getMove 深搜兜底。
    var l3Kill = [], maxKill = -1, li2;
    for (i = 0; i < bestPts.length; i++) {
      var bp = bestPts[i];
      var isL3 = false;
      for (li2 = 0; li2 < T.length; li2++) if (T[li2] === bp && isLive3End[li2]) { isL3 = true; break; }
      if (!isL3) continue;
      var k = killAtQ1(board, N, bp, opp) ? 2 : (killAtQ2(board, N, bp, me, opp) ? 1 : 0);
      if (k > maxKill) { maxKill = k; l3Kill = [bp]; }
      else if (k === maxKill) l3Kill.push(bp);
    }
    if (l3Kill.length === 1) return l3Kill;   // 恰好一个端点带杀招 → 优先占它（阻断）
    return bestPts;   // 多个带杀招 / 都不带 → 深搜兜底
  }

  // 【L7 辅助】对手落 q 是否形成**1步杀招**（活四之外的额外做杀）：
  //   落 q 成四三/四四（is43/is44）、真三三（is33 且 !isFake33，可反制的假三三不算）、
  //   或冲四+潜在四三/四四/真三三（potentialThreat >= P_D43 / P_D33 且 hasTrueTriple33Kill）。
  //   活四（LIVE4）是活三端点普遍必杀，不用于区分端点。
  function killAtQ1(board, N, q, opp) {
    var t = threatAt(board, N, q, opp, 'def');
    if (t.is43 || t.is44) return true;
    if (t.is33 && !isFake33(board, N, q, opp)) return true;
    if (t.tier === T_RUSH4) {
      var pt = potentialThreat(board, N, q, opp, 4, true);
      if (pt >= V.P_D43) return true;
      if (pt >= V.P_D33 && hasTrueTriple33Kill(board, N, q, opp)) return true;
    }
    return false;
  }

  // 【L7 辅助】对手落 q 后，q 附近 3 格内存在"冲三/活三 + 潜在四四/四三/真三三"（**2步杀招**）：
  //   对手落 q 冲四后，就近再冲三，无论堵新冲三哪边都会兑现做杀。
  function killAtQ2(board, N, q, me, opp) {
    board[q] = opp;
    var x = q % N, y = (q / N) | 0, sx, sy, has = false;
    var x0 = Math.max(0, x - 3), x1 = Math.min(N - 1, x + 3);
    var y0 = Math.max(0, y - 3), y1 = Math.min(N - 1, y + 3);
    for (sy = y0; sy <= y1 && !has; sy++) for (sx = x0; sx <= x1; sx++) {
      var s = sy * N + sx;
      if (s === q || board[s] !== EMPTY) continue;
      var tS = threatAt(board, N, s, opp, 'def');
      var ptS = potentialThreat(board, N, s, opp, 4, true);
      if ((tS.tier === T_RUSH4 || tS.tier === T_LIVE3 || tS.tier === T_LIVE3J) &&
          (ptS >= V.P_D43 || (ptS >= V.P_D33 && hasTrueTriple33Kill(board, N, s, opp)))) has = true;
    }
    board[q] = EMPTY;
    return has;
  }

  // 【L12】防对方"冲四+潜在三三"做杀（2026-08-14 从 L7 分离）：
  //   对手下 P 成冲四且落子后下一步能形成三三（P_D33 <= potentialThreat < P_D43）——
  //   **非一步必胜**（三三可防，我方可先下己方三三再应对），故优先级放在己方三三/做杀之后、
  //   防对方 VCF（L8）之后。堵点用 filterDefense（AI 落子后对手剩余该级威胁消除）。
  // ⚠ 2026-08-18 真三三校验（用户）：潜在三三须含**真三三**（is33 且 !isFake33，敌方成三三后
  //   我方可"堵活三+冲四"反制 → 假三三可化解、非必杀）才提前堵；假三三的"冲四+潜在三三"不是
  //   必杀棋，不纳入 L12 威胁（交给深搜/后续）。
  // ⚠ 2026-08-18 **致命 bug 修复**：判断 q 落 color 成三三是否因 p **新生成**——移除 p 后 q 不再成三三。
  //   向后看/VCF 中"潜在真三三"必须是关键落子 p 新引出的三三；若棋盘原本就有的三三（不依赖 p），
  //   不是 p 的"冲四+潜在真三三"威胁，不能算（否则过度防御/错误拦截）。
  function isNewTriple33(board, N, q, p, color) {
    if (board[q] !== EMPTY || board[p] !== color) return false;
    board[q] = color;
    var withP = threatAt(board, N, q, color).is33;   // p 在时 q 成三三
    board[p] = EMPTY;
    var withoutP = threatAt(board, N, q, color).is33;   // 移除 p 后
    board[p] = color;
    board[q] = EMPTY;
    return withP && !withoutP;
  }
  function hasTrueTriple33Kill(board, N, p, opp) {
    var x = p % N, y = (p / N) | 0, i, found = false;
    board[p] = opp;
    var focus = focusCells(board, N).cands;
    for (i = 0; i < focus.length; i++) {
      var q = focus[i];
      if (q === p || board[q] !== EMPTY) continue;
      var t = threatAt(board, N, q, opp, 'def');
      if (t.is33 && !isFake33(board, N, q, opp) && isNewTriple33(board, N, q, p, opp)) { found = true; break; }
    }
    board[p] = EMPTY;
    return found;
  }
  function blockRush4Potential33(board, N, me, opp) {
    var focus = focusCells(board, N).cands, T = [], i;
    for (i = 0; i < focus.length; i++) {
      var p = focus[i];
      var t = threatAt(board, N, p, opp, 'def');
      if (t.tier === T_RUSH4) {
        var pt = potentialThreat(board, N, p, opp, 4, true);
        if (pt >= V.P_D33 && pt < V.P_D43 && hasTrueTriple33Kill(board, N, p, opp)) T.push(p);
      }
    }
    if (!T.length) return [];
    return filterDefense(board, N, me, opp, T, function (t2, qi) {
      if (t2.tier !== T_RUSH4) return false;
      var pt2 = potentialThreat(board, N, qi, opp, 4, true);
      return pt2 >= V.P_D33 && pt2 < V.P_D43 && hasTrueTriple33Kill(board, N, qi, opp);
    });
  }

  // 【L8】防三三攻防一体（2026-08-18 用户：与 L9 交换——三三明显易防，VCF 隐蔽后置）：敌方已生成
  // 三三/多活三时的攻防一体堵点。用户要求：不是"有四就冲"，而是"冲四时至少彻底挡死一个活三才冲"——
  ...
  // 收集活三方向（经过 idx）的所有"堵点"空位：对手落 q 后该方向 lineType 降级（< T_LIVE3）
  //   ——即活三的两端 / 跳缺口。⚠ 不能用 collectLive3Ends（它只取相邻格，连续活三端在更远处）。
  function collectLive3Weak(board, N, idx, color, dx, dy, out) {
    var opp = color === BLACK ? WHITE : BLACK;
    var x = idx % N, y = (idx / N) | 0, seen = {};
    for (var s = -3; s <= 3; s++) {   // 活三跨 5 格，堵点（端/跳缺口）在 ±2 内，±3 足够
      var nx = x + dx * s, ny = y + dy * s;
      if (!inB(nx, ny, N)) continue;
      var q = ny * N + nx;
      if (board[q] !== EMPTY || seen[q]) continue;
      seen[q] = 1;
      board[q] = opp;
      var lt = lineType(board, N, x, y, dx, dy, color);
      board[q] = EMPTY;
      if (lt < T_LIVE3) out.push(q);
    }
  }

  // 假三三校验（2026-08-14 用户）：落 P 形成三三（is33）后，若**本次新三的任一活三方向堵点**
  //   （两端/跳缺口）被对手落子后生成新的冲四/活四（threatAt >= T_RUSH4，基于对手潜在眠三/活三），
  //   对手可"堵三三 + 反杀"一石二鸟 → 假三三。己方不优先进攻（findOwn33 跳过，交深搜判断）。
  //   ⚠ 防御不改：对手三三即使假也保守防。
  function isFake33(board, N, P, color) {
    var opp = color === BLACK ? WHITE : BLACK;
    var x = P % N, y = (P / N) | 0, k;
    var weak = [];
    board[P] = color;
    for (k = 0; k < 4; k++) {
      var dx = DIRS[k][0], dy = DIRS[k][1];
      var lt = lineType(board, N, x, y, dx, dy, color);
      if (lt === T_LIVE3 || lt === T_LIVE3J) {
        collectLive3Weak(board, N, P, color, dx, dy, weak);
      }
    }
    board[P] = EMPTY;
    var seen = {};
    for (k = 0; k < weak.length; k++) {
      ...
      if (seen[w]) continue;
      seen[w] = 1;
      if (threatAt(board, N, w, opp, 'def').tier >= T_RUSH4) return true;
    }
    return false;
  }

  // 己方做杀层辅助：落 idx 后，若"潜在三三"（下一步三三）的候选空位中**没有任一是真三三**
  // （全部 isFake33）→ 假做杀，跳过该点（避免下出会被对手破坏的三三）。
  function killPotential33Fake(board, N, idx, color) {
    board[idx] = color;
    var focus = focusCells(board, N).cands, i, hasTrue = false;
    for (i = 0; i < focus.length; i++) {
      var q = focus[i];
      var t = threatAt(board, N, q, color);
      if (t.is33 && !isFake33(board, N, q, color)) { hasTrue = true; break; }
    }
    board[idx] = EMPTY;
    return !hasTrue;
  }

  // 【L10】己方三三（落子形成 is33=双活三）。⚠ 2026-08-14：假三三（isFake33）跳过，不优先进攻，
  //   交给 L15 深搜判断（真三三深搜仍会选）。
  function findOwn33(board, N, color) {
    ...
    for (i = 0; i < focus.length; i++) {
      ...
      var t = threatAt(board, N, idx, color);
      if (t.is33) {
        if (isFake33(board, N, idx, color)) continue;
        pts.push(idx);
      }
    }
    return pts;
  ...
  function findOwnKillLayer(board, N, color) {
    ...
    for (i = 0; i < focus.length; i++) {
      ...
      var t = threatAt(board, N, idx, color);
      if (t.tier === T_RUSH4) {
        if (potentialThreat(board, N, idx, color, 4, true) >= V.P_D33) {
          if (killPotential33Fake(board, N, idx, color)) continue;
          pts.push(idx);
        }
  ...
  // 【L13】防对方潜在三三（含"活三+做杀"向后看）：
  //   ① is33 → 对方落点直接形成三三（⚠ 2026-08-18 真三三校验：假三三可被反制、非必杀 → 跳过）
  //   ② 活三(LIVE3) + potentialThreat>0 → 对方下此点成活三且为下一步组合杀铺路，同 L13 级
  ...
  function blockPotential33(board, N, me, opp) {
    ...
    for (i = 0; i < focus.length; i++) {
      ...
      if (t.is33) {
        if (isFake33(board, N, idx, opp)) continue;   // 假三三可反制，不提前堵
        T.push(idx);
      } else if ((t.tier === T_LIVE3 || t.tier === T_LIVE3J) && potentialThreat(board, N, idx, opp, 4, true) > 0) {
        T.push(idx);   // 活三+做杀（保留）
      }
    }
    if (!T.length) return [];
    return filterDefense(board, N, me, opp, T, function (t, qi) {
      if (t.is33) return !isFake33(board, N, qi, opp);
      if ((t.tier === T_LIVE3 || t.tier === T_LIVE3J) && potentialThreat(board, N, qi, opp, 4, true) > 0) return true;  // 活三+做杀
      return false;
    });
  }

  // 【L6】VCF 连续冲四强制序列搜索（可搜很深，深度14）。
  // 只扩展"落子后成五/活四/冲四"的点（威胁值高，分支极少），因此可搜很深。
  // ⚠ 理论局限：无禁手下多数"当手杀"已被 L0-L5 处理；VCF 仅在"有当手冲四且通向深杀"时触发。
  var _vcfNodes = 0, _vcfStart = 0, _vcfHard = 250;

  // 收集威胁 p 的所有对手应手（堵点）：对每个达到 minTier 的威胁方向（冲四），
  //   扫描方向线 ±4，收集"对手落 q 后该方向威胁降级"的空位（两端/跳缺口）。
  //   ⚠ 2026-08-14 allLose：VCF 必胜必须验证**所有应手**都输——任一应手能破坏链 → 非必胜。
  function getKills(board, N, p, color, minTier) {
    var opp = color === BLACK ? WHITE : BLACK;
    var x = p % N, y = (p / N) | 0, kills = [], seen = {};
    for (var k = 0; k < 4; k++) {
      var dx = DIRS[k][0], dy = DIRS[k][1];
      var lt = lineType(board, N, x, y, dx, dy, color);
      if (lt < minTier) continue;   // 非威胁方向（VCF: 只冲四）
      for (var s = -4; s <= 4; s++) {
        var nx = x + dx * s, ny = y + dy * s;
        if (!inB(nx, ny, N)) continue;
        var q = ny * N + nx;
        if (q === p || board[q] !== EMPTY || seen[q]) continue;
        seen[q] = 1;
        board[q] = opp;
        var lt2 = lineType(board, N, x, y, dx, dy, color);
        board[q] = EMPTY;
        if (lt2 < lt) kills.push(q);   // 对手落 q 破坏该方向威胁 → 应手
      }
    }
    return kills;
  }
  ...
  // 连续冲四必胜搜索（VCF）：
  //   allLose 验证：我方落 p 成冲四后，枚举对手**所有应手**（堵点），每个应手递归验证我方仍能
  //   继续冲四直到成五；任一应手能破坏（我方不胜 或 超时）→ 非必胜不下；所有应手都输 → 必胜。
  function vcfSearch(board, N, color, depth, maxDepth, minTier, collect) {
    ...
    for (i = 0; i < focus.length; i++) {
      ...
      if (t.tier === T_FIVE || t.tier === T_LIVE4 || t.is44) { board[p] = EMPTY; if (collect) collect.push(p); return p; }
      if (t.tier >= minTier) {
        var kills = getKills(board, N, p, color, minTier);
        if (kills.length === 0) { board[p] = EMPTY; if (collect) collect.push(p); return p; }   // 无堵点 → 对手无法防，必胜
        var allLose = true;
        for (var k = 0; k < kills.length; k++) {
          board[kills[k]] = opp;
          // 对手应手若形成冲四/活四（≥RUSH4）→ 我方被迫转守，连续威胁链中断 → 该应手失败
          var tB = threatAt(board, N, kills[k], opp);
          var r = (tB.tier >= T_RUSH4) ? -1 : vcfSearch(board, N, color, depth + 1, maxDepth, minTier, collect);
          board[kills[k]] = EMPTY;
          if (r < 0) { allLose = false; break; }
        }
        if (allLose) { board[p] = EMPTY; if (collect) collect.push(p); return p; }
      }
      board[p] = EMPTY;
    }
    return -1;
  ...
  // VCF 层入口：只冲四（连续冲四必胜）；250ms/8000 节点硬限内未确认必胜返回 -1（不下）。
  function vcfWin(board, N, color, maxDepth) {
    _vcfNodes = 0; _vcfStart = _now();
    return vcfSearch(board, N, color, 0, maxDepth, T_RUSH4);
  }

  // 【L9】防多步杀（用户 2026-08-12；2026-08-14 提到防三三/做杀前——VCF 必胜 > 三三；
  // 2026-08-18 用户：与 L8 交换，防三三攻防一体优先）：
  // 检测对方是否有连续冲四必胜（VCF 杀），返回**最佳打断点**；-1 无。
  // ⚠ 2026-08-14 allLose：vcfWin 只返回"对手所有应手都输"的真必胜——可被一手破坏的对手 VCF 返回 -1（不防）。
  // ⚠ 2026-08-18 新尝试：不再固定堵敌方 VCF 首手——收集敌方必胜链所有落子位（vcfSearch collect）
  //   + 各冲四的应手缺口（getKills，即"所有冲杀的可能堵点"），逐个验证"AI 落 q 后敌方 VCF 不再必胜"
  //   （vcfWin==-1；硬限放宽到 600ms，减少"换链必胜搜不到→误判有效"），再深搜选最佳——非总堵首手。
  // ⚠ 时间可控：临时收紧 _vcfHard；调用后恢复 vcf 状态。
  function findOppVcfDefense(board, N, color) {
    var opp = color === BLACK ? WHITE : BLACK;
    var sn = _vcfNodes, ss = _vcfStart, sh = _vcfHard;
    _vcfHard = 200;
    var collect = [];
    var p = vcfSearch(board, N, opp, 0, 12, T_RUSH4, collect);
    _vcfNodes = sn; _vcfStart = ss; _vcfHard = sh;
    if (p < 0) return -1;
    // 候选：敌方必胜链所有落子位 + 各冲四缺口（"所有冲杀的可能堵点"）
    var cands = [], seen = {}, i, j;
    function add(q) { if (board[q] === EMPTY && !seen[q]) { seen[q] = 1; cands.push(q); } }
    for (i = 0; i < collect.length; i++) add(collect[i]);
    for (i = 0; i < collect.length; i++) {
      var cp = collect[i];
      if (board[cp] !== EMPTY) continue;
      board[cp] = opp;   // 敌方落 cp 成冲四后枚举其应手缺口
      var ks = getKills(board, N, cp, opp, T_RUSH4);
      board[cp] = EMPTY;
      for (j = 0; j < ks.length; j++) add(ks[j]);
    }
    if (cands.length <= 1) return p;
    // 验证有效打断点：AI 落 q 后敌方 VCF 不再必胜
    var valid = [];
    for (j = 0; j < cands.length; j++) {
      var q2 = cands[j];
      board[q2] = color;
      _vcfHard = 600;
      var r = vcfWin(board, N, opp, 12);
      _vcfHard = sh;
      board[q2] = EMPTY;
      if (r < 0) valid.push(q2);
    }
    _vcfNodes = sn; _vcfStart = ss;   // 恢复 vcf 状态
    if (!valid.length) return p;
    if (valid.length === 1) return valid[0];
    // 有效候选中深搜选最佳（评估后续局面，非固定堵首手）
    return pickBestBySearch(board, N, color, opp, valid, 1000, 6);
  }

  // 【L9-b】敌方"VCF 路上"的冲四+潜在真三三复合点拦截（2026-08-18 用户，L9 防VCF 子类）：
  //   敌方连续冲四链（VCF 路）上的冲四点 p 若同时带潜在**真三三**（敌方落 p 成冲四 + 下一步兑现
  //   真三三），该复合威胁隐蔽难防（VCF 本身不易预料），需提前拦截——占住 p 同时阻断冲四与真三三。
  //   作为 L9（防VCF）后的独立子类（在己方三三 L10 前拦截，区别于 L12 放己方三三/做杀后）。
  function findVcfTriple33Block(board, N, me, opp) {
    var focus = focusCells(board, N).cands, T = [], i, j;
    for (i = 0; i < focus.length; i++) {
      var p = focus[i];
      var t = threatAt(board, N, p, opp, 'def');
      if (t.tier === T_RUSH4) {
    ...
    if (!T.length) return -1;
    // 拦截候选：p（占住敌方冲四+真三三复合点）+ p 的冲四缺口
    var cands = [], seen = {};
    for (i = 0; i < T.length; i++) {
      var p2 = T[i];
      if (board[p2] === EMPTY && !seen[p2]) { seen[p2] = 1; cands.push(p2); }
      board[p2] = opp;
      var ks = getKills(board, N, p2, opp, T_RUSH4);
      board[p2] = EMPTY;
      for (j = 0; j < ks.length; j++) if (board[ks[j]] === EMPTY && !seen[ks[j]]) { seen[ks[j]] = 1; cands.push(ks[j]); }
    }
    if (!cands.length) return -1;
    if (cands.length === 1) return cands[0];
    return pickBestBySearch(board, N, me, opp, cands, 800, 4);
  }
  ...
  function huaPuThird(board, N, color, opp, budget) {
    ...
    //   改为从天元四个对角格中**选离白2 最远的一个斜线段**（切比雪夫距离最大），
    //   建立己方斜线棋型并尽量远离白棋，不再随机。
    if (Math.abs(px - c) > 1 || Math.abs(py - c) > 1) {
      var diag = [(c - 1) * N + (c - 1), (c + 1) * N + (c - 1), (c - 1) * N + (c + 1), (c + 1) * N + (c + 1)];
      var best = [], bestDist = -1;
      for (i = 0; i < 4; i++) {
        if (board[diag[i]] !== EMPTY) continue;
        var ddx = diag[i] % N, ddy = (diag[i] / N) | 0;
        var dist = Math.max(Math.abs(ddx - px), Math.abs(ddy - py));
        if (dist > bestDist) { bestDist = dist; best = [diag[i]]; }
        else if (dist === bestDist) best.push(diag[i]);   // 并列最远 → 随机2选1
      }
  ...
  function blackFifth(board, N, color, budget) {
    if (_enhancedDefense) return -1;   // ⚠ 2026-08-18 增强防御：敌人未近身搏杀 → 跳过纯进攻开局库，交深搜
    var opp = color === BLACK ? WHITE : BLACK;
  ...
  // 【2026-08-18 用户大修】第7手候选点 = 与己方子曼哈顿距离 ≤3 的空位（多检测一些点，覆盖更远做杀链）。
  function findSeventhCands(board, N, color) {
    var stones = [], s, q;
    for (s = 0; s < N * N; s++) if (board[s] === color) stones.push(s);
    var cands = [];
    for (q = 0; q < N * N; q++) {
      if (board[q] !== EMPTY) continue;
      var qx = q % N, qy = (q / N) | 0;
      for (s = 0; s < stones.length; s++) {
        var px = stones[s] % N, py = (stones[s] / N) | 0;
        if (Math.abs(qx - px) + Math.abs(qy - py) <= 3) { cands.push(q); break; }
      }
    }
    return cands;
  }
  // 【2026-08-18 用户】落 q 后的己方棋型价值（各方向价值之和）：
  //   斜活三3 / 横竖活三2.75 / 斜跳三2.5 / 横竖跳三2.25 / 斜眠三1.75 / 横竖眠三1.5 /
  //   斜连活二1.5 / 斜跳活二1.25 / 横竖活二1 / 横竖跳活二0.75（斜连活二用户未列，按"连>跳"取1.5）。
  function seventhValue(board, N, q, color) {
    var x = q % N, y = (q / N) | 0, k, val = 0;
    board[q] = color;
    for (k = 0; k < 4; k++) {
      var dx = DIRS[k][0], dy = DIRS[k][1];
      var t = lineType(board, N, x, y, dx, dy, color);
      var diag = (dx !== 0 && dy !== 0);
      if (t === T_LIVE3) val += diag ? 3 : 2.75;
      else if (t === T_LIVE3J) val += diag ? 2.5 : 2.25;
      else if (t === T_SLEEP3) val += diag ? 1.75 : 1.5;
      else if (t === T_LIVE2) {
        var jump = isJumpLive2(board, N, x, y, dx, dy, color);
        if (diag) val += jump ? 1.25 : 1.5;
        else val += jump ? 0.75 : 1;
      }
    }
    board[q] = EMPTY;
    return val;
  }
  // 第7手价值选点：价值最高优先，并列才深搜。
  function pickBestSeventh(board, N, color, opp, cands, budget) {
    var bestVal = -1, best = [], i, v;
    for (i = 0; i < cands.length; i++) {
      v = seventhValue(board, N, cands[i], color);
      if (v > bestVal) { bestVal = v; best = [cands[i]]; }
      else if (v === bestVal) best.push(cands[i]);
    }
    if (best.length === 1) return best[0];
  ...
  // 【第7手】黑进攻（2026-08-18 用户大修）：
  //   ① 优先三活二（l2>=3）→ 深搜选；
  //   ② 无 → 对距离≤2 内所有候选点按落子后棋型价值打分（斜活三3>横竖活三2.75>...），
  //      选价值最高点；多个并列 → 深搜。去掉原"斜活三优先"与"双活二"档（价值打分覆盖）。
  function blackSeventh(board, N, color, budget) {
    if (_enhancedDefense) return -1;   // ⚠ 2026-08-18 增强防御：敌人未近身搏杀 → 跳过纯进攻开局库，交深搜
    var opp = color === BLACK ? WHITE : BLACK;
    var tri = findTripleLive2Points(board, N, color);
    if (tri.length) return pickBestBySearch(board, N, color, opp, tri, budget);
    var cands = findSeventhCands(board, N, color);
    if (!cands.length) return -1;
    return pickBestSeventh(board, N, color, opp, cands, budget);
  }

  // 【L14b】强威胁棋型识别库（2026-08-18 用户）：落 q 后形成**1 活三（含跳活三）+ 2 活二/眠三**
  //   （含跳活二/跳眠三）【活二+眠三辅助方向>=2，共 3 个有威胁方向】→ 强做杀潜力。
  //   我方落 q 成强棋型 → 优先下；对手落 q 成强棋型 → 优先堵。位于开局决策层后、深搜前。
  function strongShapeCount(board, N, q, color) {
    var x = q % N, y = (q / N) | 0, live3 = 0, aux = 0, k;
    board[q] = color;
    for (k = 0; k < 4; k++) {
      var t = lineType(board, N, x, y, DIRS[k][0], DIRS[k][1], color);
      if (t === T_LIVE3 || t === T_LIVE3J) live3++;
      else if (t === T_LIVE2 || t === T_SLEEP3) aux++;
    }
    board[q] = EMPTY;
    return (live3 >= 1 && aux >= 2);
  }
  // 强威胁库主入口：① 优先下（我方落 q 成强棋型）；② 无 → 优先堵（对手落 q 成强棋型）。返回落点或 -1。
  function findStrongThreat(board, N, me, opp) {
    var i, q;
    var myCands = findSeventhCands(board, N, me), atk = [];
    for (i = 0; i < myCands.length; i++) if (strongShapeCount(board, N, myCands[i], me)) atk.push(myCands[i]);
    if (atk.length) {
      if (atk.length === 1) return atk[0];
      return pickBestBySearch(board, N, me, opp, atk, 800, 4);
    }
    var opCands = findSeventhCands(board, N, opp), def = [];
    for (i = 0; i < opCands.length; i++) if (strongShapeCount(board, N, opCands[i], opp)) def.push(opCands[i]);
    if (!def.length) return -1;
    if (def.length === 1) return def[0];
    return pickBestBySearch(board, N, me, opp, def, 800, 4);
  }
  ...
  function openingDecision(board, N, color, budget) {
    ...
    if (color === BLACK) {
      ...
      if (stones === 2) {
        // ⚠ 2026-08-18 用户：黑第3手判断"敌人第2手是否远离黑第1手（未近身搏杀）"。
        //   若白第2手不在黑第1手周围8格 → 敌人自下棋 → 设"增强防御"标识（仅单局有效），
        //   跳过第7手纯进攻开局库 + 跳过第9手前的强制进攻导向（正常攻守转换）。
        var bi = -1, wi2 = -1, i2;
        for (i2 = 0; i2 < N * N; i2++) {
          if (board[i2] === color) bi = i2;
          else if (board[i2] === opp) wi2 = i2;
        }
        if (bi >= 0 && wi2 >= 0) {
          var _bx = bi % N, _by = (bi / N) | 0, _wx = wi2 % N, _wy = (wi2 / N) | 0;
          if (Math.abs(_wx - _bx) > 1 || Math.abs(_wy - _by) > 1) _enhancedDefense = true;
        }
        return huaPuThird(board, N, color, opp, budget);
      }
  ...
  // ═══ 置换表（Transposition Table，2026-08-14 深度搜索性能优化）═══
  // Zobrist 哈希缓存"棋盘状态 + 当前走棋方"，negamax 命中已搜过的棋局直接复用评估值，
  // 避免重复搜索相同棋型（迭代加深/多候选选点时命中率高），显著降低深度搜索耗时。
  // 确定性 xorshift32 PRNG（固定种子）保证跨运行 hash 一致、可复现。
  var TT_ENABLED = true;                             // 置换表总开关（性能对比/回退用）
  // 每层候选截断基数（用户 2026-08-14 探索广度/深度平衡）：negamax 内部与 L15 顶层共用，
  //   控制每层搜索候选数（cap）。L15 顶层白棋 = _capBase+1（保冲四2活三2活二1）。默认 10。
  var _capBase = 10;
  var TT_SIZE = 1 << 18, TT_MASK = TT_SIZE - 1;      // 262144 槽
  var TT_EXACT = 0, TT_LOWER = 1, TT_UPPER = 2;      // 精确值 / 下界(≥beta) / 上界(≤alpha)
  var _zob = [], _colorZob = [0, 0, 0], _boardHash = 0, _seed = 0x9e3779b9;
  var _enhancedDefense = false;   // ⚠ 2026-08-18 增强防御标识：敌人未近身搏杀时启用（仅单局有效）
  function _rand32() {
    _seed ^= _seed << 13; _seed ^= _seed >>> 17; _seed ^= _seed << 5;
    return (_seed >>> 0);
  }
  (function initZobrist() {
    for (var i = 0; i < 289; i++) _zob[i] = [_rand32(), _rand32(), _rand32()];  // 索引0空 / 1黑 / 2白
    _colorZob[1] = _rand32(); _colorZob[2] = _rand32();
  })();
  var _tt = new Array(TT_SIZE);   // 每槽 {key,depth,score,flag}；跨 getMove 保留，key 校验碰撞即 miss
  function computeBoardHash(board, N) {
    var h = 0, i;
    for (i = 0; i < N * N; i++) if (board[i]) h ^= _zob[i][board[i]];
    return h >>> 0;
  }
  function ttLookup(key, depth, alpha, beta) {
    var e = _tt[key & TT_MASK];
    if (e && e.key === key && e.depth >= depth) {
  ...
  function ttStore(key, depth, score, flag) {
    _tt[key & TT_MASK] = { key: key, depth: depth, score: score, flag: flag };
  }

  // 启发式·着法生成与排序（L15 深搜的起点，negamax 每层也调用）：
  // 曼哈顿≤2 空位，排序分 = 己方进攻(atk) + 堵对方(tO×t0w) + 局部棋型密度(lst)
  //   + 潜在组合杀差(×0.1) + 中心度(微弱) + 极早斜线加权(+100000)。
  // ⚠ applyDefense=true 仅顶层（L15 顶层 genMoves）——negamax 内部不传，避免 defendMode/lst
  ...
  function genMoves(board, N, color, applyDefense, applyLst, capOpt) {
    ...
    //   - 斜线：AI 前两手(<4子) 尽量走斜线（另由 getMove L14 开局决策层硬约束保证）
    ...
    // 在强制层不触发、走 L15 深搜时生效——若敌方已有威胁明显占优，落子侧重防守。
    ...
    for (i = 0; i < cands.length; i++) {
      ...
      // 黑进攻导向（用户 2026-08-12）：黑堵白价值降权×0.7 仅"白威胁不占优"且 L15 启发层
      //   （applyLst）生效；攻守转换（oppThreat>myMax×2）黑 t0w=1 正常防守。
      // ⚠ 用户 2026-08-14：AI 执黑前 9 手（黑落子 ≤5 次）不攻守转换——强制 t0w=0.7 进攻，
      //   第 9 手后才按"白威胁 > 黑进攻×8"判断。
      // ⚠ 2026-08-18 增强防御：敌人未近身搏杀 → 跳过前 9 手强制进攻（按白威胁正常攻守转换）。
      var _forceAtk = !_enhancedDefense && countStones(board, N) <= 9;
      var t0w = (applyLst && color === BLACK && (_forceAtk || !(oppThreat > myMax * 8))) ? 0.7 : 1;
      var atk = tA.value;
  ...
  // ⚠ applyLst 参数贯穿递归：L15 顶层传 true（evaluateBoard 应用局部棋型密度/黑堵白降权），
  ...
  function negamax(board, N, color, depth, alpha, beta, lastIdx, opp, ply, applyLst) {
    ...
    // 置换表查询：同棋局（棋盘哈希+走棋方）已搜过且深度足够 → 直接复用评估（α-β 窗口按 flag 处理）
    var ttKey = (_boardHash ^ _colorZob[color]) >>> 0;
    var ttV = TT_ENABLED ? ttLookup(ttKey, depth, alpha, beta) : null;
    ...
    var capOpt;
    if (!applyLst || color === BLACK) capOpt = { cap: _capBase, keepRush4: 0, keepLive3: 0, keepLive2: 0 };
    else capOpt = { cap: _capBase, keepRush4: 2, keepLive3: 1, keepLive2: 1 };
    ...
    // 写入置换表（超时结果不可靠不缓存）
    if (TT_ENABLED && !_timeout) {
      var ttFlag = TT_EXACT;
      if (best <= alpha) ttFlag = TT_UPPER;
      else if (best >= beta) ttFlag = TT_LOWER;
      ttStore(ttKey, depth, best, ttFlag);
    }
    return best;
  }
    ...
    if (maxDepth > 0) {
      ...
      for (var d = 1; d <= maxDepth; d++) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          var m = arr[i].i;
          board[m] = me;
          _boardHash ^= _zob[m][me];           // 落子增量更新哈希（2026-08-18 修复：漏更新致 TT 失效）
          var v = -negamax(board, N, opp, d, -INF, INF, m, me, 1);
          _boardHash ^= _zob[m][me];           // 撤销还原哈希
          board[m] = EMPTY;
  ...
  // 分层漏斗按固定顺序逐层检测（L0~L15），每层"触发即返回"；全部不触发才进 L15 深搜。
  //   执行顺序：L0成五 → L1防活四 → L2防冲四 → L3己方四三四四 → L4己方活四 → L5冲四做杀
  //            → L6己方VCF → L7防一步必胜 → L8防对方VCF → L9防三三攻防一体 → L10己方三三
  //            → L11己方做杀 → L12防对方冲四+潜在三三 → L13防潜在三三 → L14开局定式 → L15 深搜
  // 层内候选点统一用 pickBestBySearch 做局部 Negamax 选最优；trace 回填 {layer,elapsed,depth}。
  // 【纯深搜】跳过全部决策树层（L0-L14b），只做启发式深搜（提取自 getMove L15）。关闭决策树时用。
  function deepSearch(board, N, color, budget) {
    var opp = color === BLACK ? WHITE : BLACK;
    _start = _now(); _timeout = false; _nodes = 0;
    _boardHash = computeBoardHash(board, N);
    _hardLimit = Math.max(budget, 2000);
    var moves = genMoves(board, N, color, true, true,
      color === BLACK
        ? { cap: _capBase, keepRush4: 0, keepLive3: 0, keepLive2: 0 }
        : { cap: _capBase + 1, keepRush4: 2, keepLive3: 2, keepLive2: 1 });
    ...
    for (d = 1; d <= MAX_ITER_DEPTH; d++) {
      ...
      for (var i = 0; i < moves.length; i++) {
        ...
        var m = moves[i];
        board[m] = color;
        _boardHash ^= _zob[m][color];
        var v = -negamax(board, N, opp, d - 1, -INF, -alpha, m, color, 1, true);
        _boardHash ^= _zob[m][color];
        board[m] = EMPTY;
  ...
  // 【L-1，仅关闭决策树时有效】堵5（对手活四/冲四成五点）+ 首部天元。其余 L0-L15 仅决策树启用时有效。
  function L1Defense(board, N, color) {
    var opp = color === BLACK ? WHITE : BLACK;
    if (countStones(board, N) === 0) return (N / 2 | 0) * N + (N / 2 | 0);   // 首部天元
    var winPts = findWinPoints(board, N, opp);   // 对手落子即成五的点（活四两端/冲四空端/跳四缺口）
    if (!winPts.length) return -1;
    if (winPts.length === 1) return winPts[0];
    return pickBestBySearch(board, N, color, opp, winPts, 1000, 4);
  }

  function getMove(board, size, color, timeBudgetMs, trace, decisionTree) {
    size = Math.max(5, size | 0);
    var useDT = (decisionTree === undefined) ? true : !!decisionTree;
    var opp = color === BLACK ? WHITE : BLACK;
    var budget = (typeof timeBudgetMs === 'number' && timeBudgetMs > 0) ? timeBudgetMs : DEFAULT_BUDGET;
    var _t0 = _now();
    _boardHash = computeBoardHash(board, size);   // 置换表初始哈希（negamax 内部增量维护）
    if (countStones(board, size) === 0) _enhancedDefense = false;   // 新局重置增强防御标识（仅单局有效）
    if (!useDT) {
      // ⚠ 2026-08-18 关闭决策树：仅 L-1（堵5/首部天元）+ 深搜，跳过 L0-L15 决策树层
      var l1 = L1Defense(board, size, color);
      if (l1 >= 0) return ret(l1 % size, (l1 / size) | 0, 'L-1');
      var pd = deepSearch(board, size, color, budget);
      if (trace) { trace.layer = 'L15'; trace.elapsed = _now() - _t0; }
      return pd;
    }
    ...
    //   本就快（内部再限 2s）。L15 深搜用满整个 budget（相对 L15 开始，不扣前面层耗时）、至少 2s，
    ...
    // 【L6】己方VCF连续冲四杀（2026-08-14 提升到防一步必胜前）：连续冲四强制链到最后成五——
    //   每步冲四对手只能唯一应，全程被压制，属确认必胜。若中途形成活四/活三/四三/四四也能继续
    //   冲四直到成五。放在"防一步必胜"之前：我方必胜优先，VCF 强制链压制对手一步威胁。
    var vcfMove = vcfWin(board, size, color, 14);
    if (vcfMove >= 0) return ret(vcfMove % size, (vcfMove / size) | 0, 'L6');

    // 【L7】防对方一步必胜威胁（含"冲四+做杀"向后看）：找到能消除全部威胁的堵点
    //   - is43/is44/活四 → 一步必胜；冲四+potentialThreat>0 → 冲四为做杀铺路，同 L7 级，提前堵冲四点
    var l6 = blockCombined(board, size, color, opp);
    if (l6.length) {
      // L7 多候选堵点需"往后看"区分（选堵哪个端点能拖住/反杀），用迭代加深（预算内尽量深）
      var m6 = pickBestBySearch(board, size, color, opp, l6, remain(), MAX_ITER_DEPTH);
      return ret(m6 % size, (m6 / size) | 0, 'L7');
    }
    // 【L8】防三三攻防一体（2026-08-18 用户：与 L9 交换——三三明显易防，VCF 隐蔽后置）：
    //   敌方已生成三三/多活三时，找"堵活三又冲四"的点抢先手
    var d43 = findDefendWithRush4(board, size, color, opp);
    if (d43 >= 0) return ret(d43 % size, (d43 / size) | 0, 'L8');

    // 【L9】防对方VCF杀（原 L8，2026-08-18 交换到防三三攻防后）：己方无必胜但对方有连续冲四必胜
    //   （VCF 杀）时，占住对方杀棋首手打断
    var oppDef = findOppVcfDefense(board, size, color);
    if (oppDef >= 0) return ret(oppDef % size, (oppDef / size) | 0, 'L9');

    // 【L9-b】敌方"VCF 路上"冲四+潜在真三三复合点拦截（2026-08-18 用户，L9 防VCF 子类）：
    //   敌方冲四链上的 p 同时带潜在真三三（隐蔽难防），提前占住 p 阻断冲四与真三三（己方三三 L10 前）
    var v33 = findVcfTriple33Block(board, size, color, opp);
    if (v33 >= 0) return ret(v33 % size, (v33 / size) | 0, 'L9b');

    // 【L10】己方三三：能形成双活三就下（追平/反攻）。三三非必赢，多候选需深搜比较后续发展
    var l8 = findOwn33(board, size, color);
    if (l8.length) {
      var m8 = pickBestBySearch(board, size, color, opp, l8, remain(), MAX_ITER_DEPTH);
      return ret(m8 % size, (m8 / size) | 0, 'L10');
    }

    // 【L11】己方做杀层：① 冲四+潜在三三；② 活三+潜在四三/四四/三三（半径4检测做杀链）
    var l8b = findOwnKillLayer(board, size, color);
    if (l8b.length) {
      var m8b = pickBestBySearch(board, size, color, opp, l8b, remain(), MAX_ITER_DEPTH);
      return ret(m8b % size, (m8b / size) | 0, 'L11');
    }

    // 【L12】防对方"冲四+潜在三三"做杀（2026-08-14 从 L7 分离）：非一步必胜（三三可防），
    //   我方先下己方三三/做杀后再处理此威胁。堵点深搜选（攻防一体优先）。
    var rpt = blockRush4Potential33(board, size, color, opp);
    if (rpt.length) {
      var mrpt = pickBestBySearch(board, size, color, opp, rpt, remain(), MAX_ITER_DEPTH);
      return ret(mrpt % size, (mrpt / size) | 0, 'L12');
    }

    // 【L13】防对方潜在三三（含"活三+做杀"向后看）：对方某点落子即成三三 / 活三为做杀铺路。
    ...
    if (l9.length) {
      ...
      return ret(m9 % size, (m9 / size) | 0, 'L13');
    }

    // 【L14】开局决策层：前7手定式（合并升级原 L1 空盘天元 / L9 开局进攻 / L12b 强制斜线），
    // 位于 L15 深搜前。强制层（L0-L13）均不触发时，由本层提供开局定式：执黑 1/3/5/7 手、
    // 执白 2/4/6 手。返回 -1 表示无开局定式，交给 L15 深搜兜底。
    var od = openingDecision(board, size, color, remain());
    if (od >= 0) return ret(od % size, (od / size) | 0, 'L14');

    // 【L14b】强威胁棋型识别库（2026-08-18 用户，开局库后深搜前）：落 q 形成"1活三+2活二/眠三"
    //   强做杀潜力——我方成强棋型 → 优先下；对手成强棋型 → 优先堵。
    var st = findStrongThreat(board, size, color, opp);
    if (st >= 0) return ret(st % size, (st / size) | 0, 'L14b');

    // 【L15】启发式深度搜索（兜底层，所有强制层都不触发时）：迭代加深 1→MAX_ITER_DEPTH + 时间硬上限
    _start = _now(); _timeout = false; _nodes = 0;
    ...
    //   现预算上限设一次为整个 budget（相对 L15 开始 _start，不扣前面决策层耗时；用户 2026-08-14
    //   定稿：L15 用满预算、至少 2s，允许总时长超过单步限制，确保 AI 能力强大优先）。
    _hardLimit = Math.max(budget, 2000);
    ...
    var moves = genMoves(board, size, color, true, true,
      color === BLACK
        ? { cap: _capBase, keepRush4: 0, keepLive3: 0, keepLive2: 0 }   // 黑：不分桶（纯分数）
        : { cap: _capBase + 1, keepRush4: 2, keepLive3: 2, keepLive2: 1 }); // 白：分桶（保2/2/1 补齐）
    if (!moves.length) { if (trace) { trace.layer = 'L15'; trace.elapsed = _now() - _t0; } return null; }
    var best = moves[0], d;
    ...
    // ⚠ 超时判断用 `_now()-_start`（L15 自身耗时）而非 `_now()-_t0`（getMove 总耗时）——前面决策层
    //   耗时不被扣减，L15 始终用满整个预算（总时长允许超单步限制，用户 2026-08-14）。
    for (d = 1; d <= MAX_ITER_DEPTH; d++) {
      if (_now() - _start > budget) break;
      var alpha = -INF, depthBest = -1;
      for (var i = 0; i < moves.length; i++) {
        ...
        var m = moves[i];
        board[m] = color;
        _boardHash ^= _zob[m][color];          // 落子增量更新哈希（2026-08-18 修复：根循环漏更新致 TT 失效）
        // L15 顶层应用启发式评估（applyLst=true）：evaluateBoard 纳入局部棋型密度（黑白对称）
        // ⚠ 2026-08-18 修复窗口：原 -INF,alpha 在首候选 alpha=-INF 时传 negamax beta=-INF（alpha>=beta 非法窗口）
        //   首候选退化单支线；改用 -INF,-alpha（标准 negamax 根负窗口）：首候选 -alpha=INF 全窗口完整搜索，
        //   后续候选窄窗口保留根层 α-β 剪枝（同预算更深）。⚠ 勿改全窗口 -INF,INF——实测 2:4 棋力下降
        //   （根层失去剪枝、每候选都完整搜、深度变浅）
    ...
    _timeout = false;
    if (best < 0) best = moves[0];
    return ret(best % size, (best / size) | 0, 'L15');
  }
  ...
  var api = {
    ...
    blockCombined: blockCombined, blockPotential33: blockPotential33, blockRush4Potential33: blockRush4Potential33,
    ...
    potentialThreat: potentialThreat, vcfWin: vcfWin, getKills: getKills, findVcfTriple33Block: findVcfTriple33Block,
    genMoves: genMoves, negamax: negamax, evaluateBoard: evaluateBoard,
    // 置换表开关 + 节点计数 + 候选截断基数（性能对比/调参用；生产默认）
    setTTEnabled: function (v) { TT_ENABLED = !!v; },
    getNodes: function () { return _nodes; },
    setCapBase: function (v) { _capBase = Math.max(6, Math.min(20, v | 0)); },
    clearTT: function () { _tt = new Array(TT_SIZE); },
    probe: function (board, N, color, m, depth, budgetMs, applyLst) {
      _start = _now(); _timeout = false; _hardLimit = budgetMs;
      var opp = color === BLACK ? WHITE : BLACK;
      _boardHash = computeBoardHash(board, N);   // 置换表哈希：先同步棋盘，再落 m
      board[m] = color;
      _boardHash ^= _zob[m][color];
